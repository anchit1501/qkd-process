...
#BASIS STUFF
    ...
    def get_other_basis(self, insecure_comm_channel):
        ...
        if self.PHOTON_PULSE_SIZE == len(insecure_comm_channel.basis_check):
            self.other_basis = copy.deepcopy(insecure_comm_channel.basis_check)
            insecure_comm_channel.basis_check = []
...
#BIT AND SUBBIT STRING STUFF
    ...
    def get_sub_bit_string(self, insecure_channel):
        ...
        if len(insecure_channel.sub_shared_key) > 0:
            self.sub_shared_key = copy.deepcopy(insecure_channel.sub_shared_key)
            insecure_channel.sub_shared_key = []
        else:
    ...
    def send_pulse_sub_bit_string(self, insecure_comm_channel):
        ...
        insecure_comm_channel.sub_shared_key = []
        if len(self.shared_key) > 0:
            insecure_comm_channel.sub_shared_key = copy.deepcopy((self.shared_key[0: (int((len(self.shared_key))/2))]))
